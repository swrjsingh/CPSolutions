...
ll bin_exp(ll a, ll x, ll md){
    if(x==0) return 1;
    ll ans = bin_exp(a,x/2, mdd);
    ans = (ans*ans)%md;
    return (x%2) ? (ans*a) % md : ans;
}
ll mulinv(ll a, ll mdd = md){
    return bin_exp(a, mdd-2, mdd);
}
void solve() {
    int n,a,b,p,q
    cin>>n>>a>>b>>p>>q;
    int dp1[n+1][n*n+1], dp2[n+1][n*n+1];
    ...
    for(int i =  0; i<n; i++){
        for(int j =  0; j<n; j++){
            for(int k = 1; k<=max(p,q); k++){
                //for all possible values at each
                //increment the next step probability
                //next step = next step + 1/p or 1/q

                if(k<=p) dp1[i+1][min(n-a, j+k)] = (dp1[1+1][min(n-a,j+k)]+dp1[i][j]*mulinv(p)%md)%md;
                
                if(k<=q) dp2[i+1][min(n-a, j+k)] = (dp2[1+1][min(n-a,j+k)]+dp2[i][j]*mulinv(p)%md)%md;
            }
    ...
    for(int i=1, i<=n; i++){
        ...
        ll y = 0;
        for(int j = 0; j<n-b; j++){
            y = (y*dp2[i-1][j])%md; // so all the values of j that are lesser
        }
        ans = (ans + (x*y) %md)%md; // that probability of x * probability of y is the total prob that takahashi wins at ith step
